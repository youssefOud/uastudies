...
function elh_insert_into_db() {
    ...
    <form action="#v_form" method="post" id="v_form">
        <label for="visitor_name"><h2>Veuillez entrer votre nom et prenom*</h2></label>
        <input type="text" name="visitor_name" id="visitor_name" />
        <label for="study_title"><h2>Veuillez entrer le titre de l'étude*</h2></label>
        <input type="text" name="study_title" id="study_title" maxlength="30" />
        <label for="study_description"><h2>Veuillez entrer la description de l'étude*</h2></label>
        <!--input type="text" name="study_description" id="study_description"  maxlength="100" /-->
        <textarea name="study_description" id="study_description" rows="10" cols="50" maxlength="100"></textarea>
        <input type="submit" name="submit_form" value="submit" />
    </form>
    <?php
    $html = ob_get_clean();
    // does the inserting, in case the form is filled and submitted
    if ( isset( $_POST["submit_form"] ) && $_POST["visitor_name"] != "" && $_POST["study_title"] != "" && $_POST["study_description"] != "" ) {
        $table = $wpdb->prefix."studies";
        $name = strip_tags($_POST["visitor_name"], "");
        $study_title = strip_tags($_POST["study_title"], "");
        $study_description = strip_tags($_POST["study_description"], "");
        $wpdb->insert( 
            $table, 
            array( 
                'name' => $name,
        ...
        $html = "<p>Votre étude a bien été ajoutée à la base de données</p>";
    }
    // if the form is submitted but the name is empty
    if ( isset( $_POST["submit_form"] ) && ($_POST["visitor_name"] == "" || $_POST["study_title"] == "" || $_POST["study_description"] == "") )
        $html .= "<p>Vous devez remplir tous les champs!</p>";
    // outputs everything
    return $html;
}
...
<?php
    global $wpdb;
    $result = $wpdb->get_results ( "SELECT* FROM wp_studies" );
    foreach ( $result as $print )   {
    ?>
    <tr>
    <td><?php echo $print->id;?></td>
    <td><?php echo $print->name;?></td>
    <td><?php echo $print->studyTitle;?></td>
    <td><?php echo $print->studyDescription;?></td>
    </tr>
        <?php }
    }


function delete_data_from_db() {

	global $wpdb;
    // starts output buffering
    ob_start();
    $min = $wpdb->get_var ( "SELECT min(id) FROM wp_studies" );
    $max = $wpdb->get_var ( "SELECT max(id) FROM wp_studies" );
    $min = (string)$min;
    $max = (string)$max;
    ?>
    <form action="#v_form" method="post" id="v_form">
        <label for="study_to_delete"><h2>Veuillez entrer l'id de l'étude que vous souhaitez supprimer</h2></label>
        <input type="number" name="study_to_delete" id="study_to_delete" min= "<?php echo htmlspecialchars($min); ?>" max= "<?php echo htmlspecialchars($max); ?>" />
        <input type="submit" name="submit_form" value="submit" />
    </form>
    <?php
    $html = ob_get_clean();
    // does the inserting, in case the form is filled and submitted
    if ( isset( $_POST["submit_form"] ) && $_POST["study_to_delete"] != "") {
        $table = $wpdb->prefix."studies";
        $id = strip_tags($_POST["study_to_delete"], "");
        $wpdb->delete( $table, array( 'id' => $id ) );
        $html = "<p>L'étude avec cette id a été supprimée, actualiser la page pour visualiser les changements</p>";
    }
    // if the form is submitted but the name is empty
    if ( isset( $_POST["submit_form"] ) && $_POST["study_to_delete"] == "")
        $html .= "<p>Veuillez remplir le champ</p>";
    // outputs everything
    return $html;
}
